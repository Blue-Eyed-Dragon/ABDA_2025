...
# =========================
base_model <- brm(
  formula = fraud_reported ~ age + policy_deductable + property_damage + bodily_injuries + 
    police_report_available + total_claim_sum,
  data = train_data,
...
hierarchical_model <- brm(
  formula = fraud_reported ~ property_damage + bodily_injuries + total_claim_sum + (1 | incident_state),
  data = train_data,
  family = bernoulli(link = "logit"),
  backend = "cmdstanr",
  chains = 4,
